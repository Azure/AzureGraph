...
#' @param ... Other arguments passed to `gr_graph$new()`.
#'
#' @details
...
#'
#' @return
#' For `get_graph_login` and `create_graph_login`, an object of class `gr_graph`, representing the login client. For `list_graph_logins`, a (possibly nested) list of such objects.
#'
#' If the AzureR data directory for saving credentials does not exist, `get_graph_login` will throw an error.
#'
#' @seealso
#' [gr_graph], [AzureAuth::get_azure_token] for more details on authentication methods
#'
                               ...
                               config_file=NULL, ...)
{
    ...
    client <- gr_graph$new(tenant, app, password, username, auth_type, host, aad_host, config_file, ...)
...
get_graph_login <- function(tenant="common", selection=NULL, refresh=TRUE)
{
    ...
    token <- readRDS(file)
    client <- gr_graph$new(token=token)
...
list_graph_logins <- function()
{
    ...
    logins <- sapply(graph_logins, function(tenant)
    {
        sapply(tenant, function(hash)
        {
            ...
            gr_graph$new(token=readRDS(file))
        }, simplify=FALSE)
    }, simplify=FALSE)
